...
function Header(props) {
    ...
    const [hitungCounter, setHitungCounter] = useState(1);
    ...
    async function WhiteListMint() {
        ...
        if (isWhitelist1) {
            try {
                ...
                const tx = await con.whitelistMint(hitungCounter, {
                    value: ethers.utils.parseEther(amount)
                })
                setLoading(true)
                await tx.wait()
    ...
    function decrementCount() {
        if (hitungCounter > 1) {
            ...
            toast.error("Atlest you have to mint 1 ! ", {
                theme: "dark",
            });
        }
    }
            ...
            if (loading) {
                return (
                    <div className={`text-center ${styles.modalBtnConnect}`}>
                        ...
                        <a href="#" className={`btn ${styles.btnModalConnect}`} onClick={WhiteListMint}>loading</a>
                    </div>
                )
            }
            else {
    ...
    const NormalconnectedMint = () => {
        if (connected) {
            if (loading) {
                return (
                    <div className={`text-center ${styles.modalBtnConnect}`}>
                        <a href="#" className={`btn ${styles.btnModalConnect}`} onClick={NormalMint}>loading...</a>
